...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus query_status) const {
...
private:
    ...
    vector<Document> FindAllDocuments(const Query& query, DocumentPredicate document_predicate) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto term_freq : word_to_document_freqs_.at(word)) {
        ...
        for (const string& word : query.minus_words) {
            ...
            for (const auto term_freq : word_to_document_freqs_.at(word)) {
                document_to_relevance.erase(term_freq.first);
            }
        }
        ...
        for (const auto document : document_to_relevance) {
            matched_documents.push_back(
                { document.first, document.second, documents_.at(document.first).rating });
        }
        return matched_documents;
    }
};
